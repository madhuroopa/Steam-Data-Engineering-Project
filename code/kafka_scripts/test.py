...
class DailyScript:
    ...
    def save_filtered_data(self):
        ...
        s3_key_top_20 = f's3a://{self.BUCKET_NAME}/data/cleaned_data/daily_data/top_20/'
        
        s3_key_top_free = f's3a://{self.BUCKET_NAME}/data/cleaned_data/daily_data/top_free/'
        s3_key_top_not_free = f's3a://{self.BUCKET_NAME}/data/cleaned_data/daily_data/top_not_free/'
        self.most_daily_played.write.mode("overwrite").option("header", "true").csv(s3_key_top_20)
        self.free_to_play_sorted.write.mode("overwrite").option("header", "true").csv(s3_key_top_free)

        self.not_free_to_play_sorted.write.mode("overwrite").option("header", "true").csv(s3_key_top_not_free)
